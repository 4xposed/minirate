defmodule Minirate.CounterTest do
  ...
  describe ".get_count/4 - increments by n" do
    ...
    test "updates only the count when it's called more than once" do
      ...
      assert {:ok, 4} == Counter.get_count(:rate_limiter, {"test", 1, fake_future_time}, increment)
  ...
  describe ".check_limit/3 - increments by 1 and checks if the limit has exceeded" do
    ...
    test "when the limit is lower than the count" do
      ...
      assert {:block, :limit_exceeded} == Counter.check_limit(:rate_limiter, {"test", 1, limit, fake_future_time})
    end
    ...
    test "when the limit is higher than the count" do
      ...
      assert {:allow, 16} == Counter.check_limit(:rate_limiter, {"test", 1, limit, fake_future_time})
    end
  end
  ...
  describe ".check_limit/4 - increments by n and checks if the limit has exceeded" do
    test "when there's no count yet" do
      ...
      assert {:allow, 5} == Counter.check_limit(:rate_limiter, {"test", 1, limit, fake_now}, increment)
    end
    ...
    test "when the limit is lower than the count" do
      ...
      assert {:block, :limit_exceeded} == Counter.check_limit(:rate_limiter, {"test", 1, limit, fake_future_time}, increment)
    end
    ...
    test "when the limit is higher than the count" do
      ...
      assert {:allow, 20} == Counter.check_limit(:rate_limiter,{"test", 1, limit, fake_future_time}, increment)
    end
  end
